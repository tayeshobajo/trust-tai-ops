-- Conversation continuity + long term recall.
--
-- Two records make "option B" mean something months later:
--   conversation_anchors  — the labelled choices the agent actually offered
--   message_references    — what a later message was proven to point back at
--
-- Both are written by the server only. If the browser could author either, it
-- could manufacture a "you said this earlier" the agent would then act on.
--
...
-- Re-indexing the same message converges instead of multiplying its anchors.
...
begin
  if not exists (
    ...
    where schemaname = 'public' and tablename = 'conversation_anchors'
      and policyname = 'conversation_anchors_read_same_org'
  ) then
    execute $policy$
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'message_references'
      and policyname = 'message_references_read_same_org'
  ) then
    execute $policy$
