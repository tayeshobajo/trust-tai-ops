...
create table if not exists project_environments (
  ...
  stack text not null default 'wordpress' check (stack in ('wordpress', 'meteor', 'nextjs', 'custom')),
  versions jsonb not null default '{}'::jsonb,
  runtime jsonb,
  -- Legacy WordPress-only columns. Kept nullable so older readers stay valid.
...
create table if not exists project_access_methods (
  ...
  access_type text not null check (access_type in ('wordpress_admin', 'sftp', 'ssh', 'hosting_portal', 'database', 'cdn', 'server_pm2', 'ci_cd', 'container')),
  label text not null,
...
create table if not exists runs (
  ...
  title text not null,
  task_type text not null check (task_type in ('malware', 'performance', 'broken_site', 'plugin_theme_conflict', 'hardening', 'qa_only', 'deploy', 'migration', 'feature', 'dependency_upgrade')),
