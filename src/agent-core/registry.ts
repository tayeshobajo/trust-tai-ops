...
export const TOOL_REGISTRY: Record<ToolId, ToolDefinition> = {
  ...
  "seo.pagespeed": {
    id: "seo.pagespeed",
    purpose: "Run a PageSpeed Insights audit on any URL — returns Core Web Vitals, Lighthouse scores, and performance opportunities.",
    capability: "public_internet",
    readOnly: true,
    risk: classifyRisk("seo.pagespeed"),
    implemented: true,
    validate: requireUrl,
    execute: runThroughGateway,
  },
  "seo.schema_validate": {
    id: "seo.schema_validate",
    purpose: "Validate structured data on a page — confirms schema types, errors, and rich-result eligibility via Google's Rich Results Test API.",
    capability: "public_internet",
    readOnly: true,
    risk: classifyRisk("seo.schema_validate"),
    implemented: true,
    validate: requireUrl,
    execute: runThroughGateway,
  },
  "seo.sitemap_audit": {
    id: "seo.sitemap_audit",
    purpose: "Fetch and parse the full sitemap tree — returns every listed URL, lastmod dates, and flags missing or malformed entries.",
    capability: "public_internet",
    readOnly: true,
    risk: classifyRisk("seo.sitemap_audit"),
    implemented: true,
    validate: requireUrl,
    execute: runThroughGateway,
  },
  ...
  "security.headers": {
    id: "security.headers",
    purpose: "Check HTTP security headers on a URL — grades CSP, HSTS, X-Frame-Options, referrer policy, and permissions policy.",
    capability: "public_internet",
    readOnly: true,
    risk: classifyRisk("security.headers"),
    implemented: true,
    validate: requireUrl,
    execute: runThroughGateway,
  },
